    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-3">
          <span
            className={`text-xs px-2 py-0.5 rounded-full border ${difficultyStyle[q.difficulty] || difficultyStyle.Medium}`}
          >
            {q.difficulty}
          </span>
          {open ? (
          ...
          >
            <div className="px-4 pb-4 space-y-3 border-t border-slate-700/40 pt-3">
              <div>
                ...
                <p className="text-sm text-slate-300 leading-relaxed bg-slate-800/40 rounded-xl p-3">
                  {q.sampleAnswer}
                </p>
              </div>
              {q.tip && (
...
export default function InterviewPage() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const data = await generateQuestions(resumeText, jobDescription);
      setResult(data);
  ...
  const activeQuestions = result
    ? ((result as any)[activeCategory] as Question[])
    : [];
  ...
  return (
    <div className="min-h-screen p-6 lg:p-10">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                AI-generated questions with sample answers tailored to your
                profile
              </p>
            </div>
          </div>
        ...
        {loading && (
          <LoadingSpinner
            message="Gemini AI is crafting your personalized questions..."
            size="lg"
          />
        )}
              ...
              <div className="glass-card p-4 flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-white">
                    {totalQuestions} personalized questions generated
                  </span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {["Easy", "Medium", "Hard"].map((d) => {
                    const count = Object.values(result)
                      .flat()
                      .filter((q: any) => q.difficulty === d).length;
                    return (
                      <span
                        key={d}
                        className={`text-xs px-2 py-0.5 rounded-full border ${difficultyStyle[d]}`}
                      >
                        {count} {d}
                      </span>
                    );
                ...
                {categoryConfig.map((cat) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {cat.label}
                      <span
                        className={`text-xs px-1.5 py-0.5 rounded-full ${isActive ? "" : "bg-slate-700 text-slate-400"}`}
                      >
                        {questions?.length || 0}
                      </span>
                    </button>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 mb-2">
                  {activeConfig && (
                    <>
                      <activeConfig.icon
                        className={`w-4 h-4 ${activeConfig.color}`}
                      />
                      <h2 className="font-semibold text-white text-sm">
                        {activeConfig.label} Questions
                      </h2>
